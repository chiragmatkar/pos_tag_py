...
from .tags import strip_tags ,is_tag
import wn
...
def sentence_keywords(str, top=10, nouns=True, singularize=True, filters=[]):
    ...
    for word in str:
        ...
        if nouns == True:
            if singularize \
            and len(word) > 3 \
            and wn.is_noun(singular.singular(word)):
                noun = True
                word = singular.singular(word)
            elif wn.is_noun(word):
                noun = True
        ...
        and not numerals.is_number(word) \
        and not is_tag(word) \
...
def is_emotion(word, shallow=False, pos=None, boolean=True):
    ...
    for i in indices:
        emotion = ekman[i]
        s = wn.senses(emotion, pos)
        for j in range(len(s)):
            if word in s[j]:
                return _return(commonsense_ekman[i])
            h = wn.hyponyms(emotion, j, pos)
            h = wn.flatten(h)
            if word in h:
                return _return(commonsense_ekman[i])
    ...
    if shallow and wn.lexname(word, 0, pos) == "feeling":
    ...
    for emotion in other:
        for w in wn.flatten(wn.hyponyms(emotion, 0, pos)):
            if word == w:
                return _return(emotion)
            if not shallow:
                if word in wn.flatten(wn.hyponym(w, 0, pos)):
                    return _return(w)
                    
    return _return(None)

def noun_is_emotion(word, shallow=False, boolean=True): 
    return is_emotion(word, shallow, wn.NOUNS, boolean)

def verb_is_emotion(word, shallow=False, boolean=True): 
    return is_emotion(word, shallow, wn.VERBS, boolean)

def adjective_is_emotion(word, shallow=False, boolean=True): 
    return is_emotion(word, shallow, wn.ADJECTIVES, boolean)

def adverb_is_emotion(word, shallow=False, boolean=True): 
    return is_emotion(word, shallow, wn.ADVERBS, boolean)
